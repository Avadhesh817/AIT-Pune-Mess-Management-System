...
def download_receipt(bill_no):
    try:
        ...
        if result:
            ...
            # Fetch food, quantity, and cost from the `order1` table for each order ID
            ...
            pdfmetrics.registerFont(TTFont('SegoeUI', 'C:\\Users\\Dell\\Desktop\\dbms mini project 1\\font\\segoeui.ttf'))
...
def get_calls_accepted_by_waiters():
    try:
        ...
        for row in undertaken_orders:
            print(row[0]," ",row[1])
        # Prepare the data to return as a JSON response
        orders = [{'waiter_name': row[0], 'order_id': row[1]} for row in undertaken_orders]

        # Return the data as a JSON response
        return jsonify({'acceptedCalls': orders})
    except Exception as e:
        # Handle any exceptions or errors that may occur
        return jsonify({'error': 'An error occurred: ' + str(e)})
...
def undertake_order(orderId):
    # if 'customer_username' in session:
    #             #first check is there any bill_no in bill table whose order_id is that which is in place_order whose customer_username is the session['username'] which is not there in customer_pays_bill table if so then select that bill_no into bill_no into bill_no variable , otherwise generate new  unique bill_no, and insert the bill_no and order_id into bill_table.
    #     cursor = db.cursor()
    #     select_query = """
    #         SELECT b.bill_no
    #         FROM bill b
    #         LEFT JOIN customer_pays_bill cpb
    #         ON b.bill_no = cpb.bill_no
    #         WHERE cpb.customer_username IS NULL
    #         AND b.order_id IN (
    #             SELECT order_id
    #             FROM place_order
    #             WHERE customer_username = %s
    ...
    #         LIMIT 1;
    #     """
    #     cursor.execute(select_query, (session['customer_username'],))
    #     unpaid_bill = cursor.fetchone()
    #     print(unpaid_bill)
    #     if unpaid_bill:
    #         # An unpaid bill exists, use its bill number
    #         bill_no = unpaid_bill[0]
    #     else:
    #         bill_no=generate_unique_billno()
            
    #     insert_query = "INSERT INTO bill VALUES (%s,%s)"
    #     cursor.execute(insert_query, (bill_no,orderId,))
    #     db.commit()
    if 'chef_username' in session:
        ...
        try:
            ...
            return jsonify({'status': 'error', 'message': 'Failed to undertake the order: ' + str(e)})
    else:
...
def call_waiter(orderId):
    if 'chef_username' in session:
        ...
        try:
            ...
            return jsonify({'status': 'error', 'message': 'Failed to call the waiter: ' + str(e)})
    else:
        return jsonify({'status': 'error', 'message': 'Chef is not logged in'})
...
def view_order():
    if 'customer_username' in session:
        # Fetch order details for the logged-in user from the database
        cursor = db.cursor()
        cursor.execute("SELECT order_id, cost FROM place_order WHERE customer_username = %s", (session['customer_username'],))
...
def check_awaiting_order():
    if 'chef_username' in session:
        # Fetch order details for the logged-in user from the database
        cur = db.cursor()
        cur.execute("SELECT order_id, cost FROM place_order;")
...
def generate_order_id():
    while True:
        # Generate a random alphanumeric order ID (e.g., "ab12")
        alphanumeric_chars = string.ascii_letters + string.digits
        order_id = ''.join(random.choice(alphanumeric_chars) for _ in range(4))
        
        # Check if the generated order ID is unique
        cursor = db.cursor()
        check_query = "SELECT order_id FROM order_item WHERE order_id = %s"
...
def insert_order_details(order_id, order_data, tableno):
    try:
        ...
        cursor = db.cursor()
        insert_places_query = "INSERT INTO place_order (customer_username, order_id, cost, tableno) VALUES (%s, %s, %s,%s)"
...
def process_order():
    try:
        ...
        # Generate a new alphanumeric order ID (you can use your existing method)
        order_id = generate_order_id()

        # Insert order details into the 'order' table
        if insert_order_details(order_id, order_data, tableno):
...
def place_order():
    if 'customer_username' in session:
        ...
        # Loop through the form fields to gather order items and quantities
        for item_data in request.form.getlist('order'):
        ...
        # Insert the order ID, customer username, and total cost into the 'place_order' table
        cursor = db.cursor()
        insert_places_query = "INSERT INTO place_order (customer_username, order_id, total_cost) VALUES (%s, %s, %s)"
        cursor.execute(insert_places_query, (session['customer_username'], order_id, total_cost))
        db.commit()
        #mycursor.close()
